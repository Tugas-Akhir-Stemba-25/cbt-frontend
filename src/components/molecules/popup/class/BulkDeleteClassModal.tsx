...
import { useBulkDeleteClass } from '@/http/class/bulk-delete-class'
...
interface BulkDeleteClassModalProps {
  openModal: boolean
...
const BulkDeleteClassModal = ({ openModal, setOpen, ids, classKey }: BulkDeleteClassModalProps) => {
  // Query Client
  const queryClient = useQueryClient()

  // Mutation
  const { mutate, isPending } = useBulkDeleteClass({
    onSuccess: (res) => {
      setOpen(false)
...
export default BulkDeleteClassModal
