...
const ExamCreate = () => {
  ...
  const { mutate: createTest, isPending: createTestPending } = useCreateTest({
    onSuccess: (data) => {
      ...
      toast.success('Sukses', {
        ...
        action: {
          label: 'Detail Ujian',
          onClick: () => {
            router.push(`/dashboard/admin/exam/${data.data.id}`)
          }
        }
      })
    },
    onError: (error) => {
      toast.error('Gagal', {
        description: error.response?.data.meta.message || error.response?.data.meta.error
      })
    }
  })
          ...
          content={
            <>
              <TabsContent value="detail">
                <ExamDetailForm form={formDetail} setTab={setTab} />
              </TabsContent>
              <TabsContent value="settings">
                <ExamSettingForm form={formSetting} onSubmit={onSubmit} isLoading={createTestPending} />
              </TabsContent>
            </>
          }
        />
